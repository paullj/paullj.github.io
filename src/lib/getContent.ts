...
const request = requestWithoutAuth.defaults({
	headers: {
		authorization: 'token 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'
	}
});

const getComments = async (issueNumber) => {
	const results = await request('GET /repos/{owner}/{repo}/issues/{issue_number}/comments', {
		owner: GITHUB_USER,
		repo: GITHUB_REPO,
